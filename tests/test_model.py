...
def test_model_forward_shape_offline(monkeypatch) -> None:
    def fake_create_model(*args, **kwargs):
        return _DummyBackbone(num_classes=kwargs["num_classes"])

    def fake_resolve_config(_model):
    ...
    monkeypatch.setattr(model_mod, "resolve_model_data_config", fake_resolve_config)
